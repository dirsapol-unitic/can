            ...
            <div class="box-body">
                ...
                @foreach($estimacions as $key => $estimacion)
                    ...
                    <!--table id="example" class="stripe row-border order-column" cellspacing="0" -->  
                        <thead>
                            <tr>
                                @if($condicion_boton==2 || $condicion_boton==3)
                    ...
                    ?>
                            ...
                            <tr>
                                @if($condicion_boton==2)
                                    <td>
                                        <?php echo "<a data-toggle='tooltip' data-original-title='Ver Producto!' onclick='ver_datos(\" ".$estimacion->id."\" ,\"".$estimacion->descripcion."\" )' class='btn btn-info btn-xs'><i class='glyphicon glyphicon-eye-open'></i></a>";
                                        ?>
                                    </td>
                                @else
                                    <td>
                                    <?php echo "<a data-toggle='tooltip' data-original-title='Ver Producto!' onclick='ver_ipress(\" ".$estimacion->petitorio_id."\" ,\"".$estimacion->descripcion."\" )' class='btn btn-info btn-xs'><i class='glyphicon glyphicon-eye-open'></i></a>";
                                    ?>
                                </td>
                                @endif
