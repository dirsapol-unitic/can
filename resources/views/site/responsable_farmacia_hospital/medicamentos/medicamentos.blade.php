...
@section('content')
  @if($num_estimaciones>0)
    @if (Auth::user()->rol>=3 and Auth::user()->rol<=6 || Auth::user()->rol==8)
        <div class="content">
            <div class="clearfix"></div>
            @include('flash::message')
            <div class="clearfix"></div>
            <a href="{!! route('farmacia_servicios.index') !!}" class='btn btn-info'><i class="glyphicon glyphicon-hand-left"></i> Regresar 2</a>
            <br/><br/>
            <div class="box box">
                <div class="box-body">
                    <h3 class="pull-left">Listado de Productos</h3>
                      <h1 class="pull-right">
                        <a target="_blank" data-toggle="tooltip" title="Listar Productos por Servicio!" href="{!! route('farmacia_servicios.productos_servicio_tipo',[$can_id,$tipo]) !!}" class='btn bg-green btn-flat btn-lg'><i class="fa fa-file-excel-o"></i> Ver Productos por Servicios </a>  
                        @if($cerrado_medicamento==0)                          
                            <a class="btn bg-red btn-flat btn-lg" onclick="CerrarPetitorio({!!$can_id!!},{!!$establecimiento_id!!},{!!$tipo!!})"> <i class="fa fa-lock"></i> Cerrar Petitorio</a>
                        @endif
                      </h1>
                    <div class="row">
                        <div class="col-xs-12">        
                            <div class="box-body">
                                <table id="estimacion" class="table table-striped">
                                    <thead>
                                        <tr>
                                            <th rowspan="2" style="text-align:center;">Ver</th>
                    ...
                    @include('site.responsable_farmacia_hospital.medicamentos.form')
...
<!-- Bootstrap core JavaScript
    ...
    <script type="text/javascript">
      var table = $('#estimacion').DataTable({
                      ...
                      ajax: "{{ route('farmacia_servicios.cargar_datos_consolidado',['estimacion'=>$can_id, 'establecimiento_id'=>$establecimiento_id,'tipo'=>$tipo]) }}",
      ...
      function editForm(id) {
        ...
        $.ajax({
          url: "{{ url('farmacia_servicios/') }}" + '/' + id + "/edit",
          type: "GET",
          dataType: "JSON",
      ...
      function CerrarPetitorio(can_id,establecimiento_id,tipo){
          ...
          swal({
              ...
              confirmButtonText: 'Si, cerrar!'
               ...
               window.location.href = '/farmacia_servicios/estimacion/cerrar_medicamento_consolidado/' + can_id + '/' + establecimiento_id + '/'+ tipo;
          });
        }
        ...
        function deleteForm(id){
          ...
          swal({
              ...
              $.ajax({
                  url : "{{ url('farmacia_servicios/eliminar')}}" + '/' + id,
                  type : "POST",
      ...
      $(function(){
            ...
            if (!e.isDefaultPrevented()){
                ...
                var id = $('#id').val();
                url = "{{ url('farmacia_servicios/grabar') . '/' }}" + id;
